    ...
    return Scaffold(
      ...
      body: Center(
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const Text('One App Any Business',
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(children: [
          const Text('What type of business do you run?',
...
class _BusinessSetupScreenState extends State<BusinessSetupScreen> {
  ...
  Future<void> _getGps() async {
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Location captured: Johannesburg')),
      );
    }
  }
  ...
  Future<void> _save() async {
    ...
    if (!_phone.text.startsWith('+27')) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Phone must start with +27')),
      );
      return;
    }
    final p = await SharedPreferences.getInstance();
    await p.setString('first_open',
        p.getString('first_open') ?? DateTime.now().toIso8601String());
        ...
        TextField(
          ...
          decoration: InputDecoration(
              ...
              suffixIcon: IconButton(
                  icon: const Icon(Icons.gps_fixed), onPressed: _getGps)),
        ),
        const SizedBox(height: 16),
        OutlinedButton.icon(
            onPressed: _pickLogo,
    ...
    if (!paid && (left == 27 || left == 29 || left == 30)) {
      ...
      showDialog(
        ...
        builder: (_) => AlertDialog(
          ...
          content:
              Text('$left days left. Subscribe now to keep using BizBox SA.'),
          actions: [
            TextButton(
        ...
        return Scaffold(
                ...
                child: Center(
                  child: Text(
                    ...
                    style: TextStyle(
                      color: paid
                          ? Colors.green
                          : (left <= 5 ? Colors.red : Colors.black),
                      fontWeight: FontWeight.bold,
                    ),
                  ),
                ),
          ...
          body: Column(children: [
            Container(
              ...
              color: paid
                  ? Colors.green
                  : (left <= 5 ? Colors.red : Colors.green.shade400),
              padding: const EdgeInsets.all(8),
              child: Text(
                paid ? '✅ Subscription Active' : '⏳ Trial: $left days left',
                textAlign: TextAlign.center,
          ...
          bottomNavigationBar: NavigationBar(
            ...
            destinations: const [
              ...
              NavigationDestination(icon: Icon(Icons.people), label: 'Debt'),
  ...
  Widget build(BuildContext context) {
    return ListView(padding: const EdgeInsets.all(16), children: [
      ...
      FilledButton.icon(
        ...
        label: const Text('Share Report via WhatsApp'),
        onPressed: () {
...
class _DebtPageState extends State<DebtPage> {
  ...
  Future<void> _load() async {
    final db = await DBHelper.database;
    setState(() => _rows = await db.query('debts'));
  }
    ...
    await showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Column(mainAxisSize: MainAxisSize.min, children: [
          TextField(controller: n, decoration: const InputDecoration(labelText: 'Customer')),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _rows.isEmpty
          ? const Center(child: Text('No debts yet'))
          : ListView.builder(
              itemCount: _rows.length,
              itemBuilder: (c, i) {
                final r = _rows[i];
                final bal = (r['amount'] as num) - (r['paid'] as num);
                return ListTile(
                  title: Text(r['customer']),
                  subtitle: Text('${r['phone']} • Balance: R$bal'),
                  trailing: FilledButton(
                    onPressed: () async {
                      final db = await DBHelper.database;
                      await db.update('debts', {'paid': r['amount']},
...
class _ExpensePageState extends State<ExpensePage> {
  ...
  Future<void> _load() async {
    final db = await DBHelper.database;
    setState(() => _rows = await db.query('expenses'));
  }
      ...
      body: _rows.isEmpty
          ...
          : ListView.builder(
              ...
              itemBuilder: (c, i) => ListTile(
                ...
                trailing: Text('R${_rows[i]['amount']}'),
              ),
            ),
    );
  }
...
class SettingsPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<SharedPreferences>(
      ...
      builder: (c, s) {
        ...
        final p = s.data!;
        final lang = p.getString('lang') ?? 'en';
        final hasLock = (p.getString('owner_lock_pin') ?? '').isNotEmpty;
        return ListView(children: [
          ListTile(
            leading: const Icon(Icons.language),
            title: const Text('Language'),
            subtitle: Text(lang == 'en' ? 'English' : 'isiZulu'),
            trailing: Switch(
              value: lang == 'zu',
              onChanged: (v) async {
                await p.setString('lang', v ? 'zu' : 'en');
          ...
          ListTile(
            ...
            title: Text(hasLock ? 'Change Staff Lock PIN' : 'Set Staff Lock PIN'),
            subtitle: Text(hasLock
                ? 'Staff must enter PIN to open app'
                : 'Protect the app with a 4-6 digit PIN'),
            onTap: () => _setOwnerPin(context, p),
          ),
          if (hasLock)
            ListTile(
              leading: const Icon(Icons.lock_open),
              title: const Text('Remove Staff Lock PIN'),
              onTap: () async {
                await p.remove('owner_lock_pin');
                if (context.mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text('Staff Lock removed')));
                  (context as Element).markNeedsBuild();
                }
              },
            ),
          ...
          ListTile(
            ...
            onTap: () => _showText(context, 'Privacy Policy',
                'BizBox SA respects your privacy. All data stays on your device.'),
          ),
          ListTile(
            leading: const Icon(Icons.description),
            title: const Text('Terms & Conditions'),
            onTap: () => _showText(context, 'Terms',
                'By using BizBox SA you agree to our terms. R99/month subscription.'),
          ),
          ListTile(
            leading: const Icon(Icons.support_agent),
            title: const Text('Contact Support'),
            onTap: () => launchUrl(
                Uri.parse('https://wa.me/27000000000?text=Help%20BizBox')),
          ),
          ListTile(
          ...
          ListTile(
            ...
            onTap: () async {
              ...
              if (context.mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(content: Text('Checking Google Play...')));
              }
            },
          ),
          ListTile(
          ...
          ListTile(
            ...
            subtitle: const Text('Simulated 6-digit OTP'),
            onTap: () => _otp(context),
          ),
        ]);
      },
  ...
  void _showText(BuildContext c, String t, String m) => showDialog(
      context: c, builder: (_) => AlertDialog(title: Text(t), content: Text(m)));
  ...
  Future<void> _setOwnerPin(BuildContext c, SharedPreferences p) async {
    ...
    await showDialog(
      context: c,
      builder: (_) => AlertDialog(
        title: const Text('Set Staff Lock PIN'),
        content: TextField(
          controller: pin,
          keyboardType: TextInputType.number,
          obscureText: true,
          maxLength: 6,
          decoration: const InputDecoration(
              hintText: '4-6 digits', border: OutlineInputBorder()),
        ),
        actions: [
          TextButton(
          ...
          FilledButton(
            onPressed: () async {
              ...
              if (v.length >= 4 && v.length <= 6) {
                await p.setString('owner_lock_pin', v);
                if (c.mounted) {
                  Navigator.pop(c);
                  ScaffoldMessenger.of(c).showSnackBar(
                      const SnackBar(content: Text('Staff Lock PIN saved ✓')));
                }
              } else {
                ScaffoldMessenger.of(c).showSnackBar(
...
class _PaymentScreenState extends State<PaymentScreen> {
    ...
    try {
      ...
      if (!started && mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text(
              'Product not available. Create "bizbox_monthly_99" in Play Console first.'),
        ));
      }
    } catch (e) {
      if (mounted) {
  ...
  Future<void> _uploadEft() async {
    ...
    if (x != null && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('EFT proof uploaded. We will confirm within 24h.')));
    }
  }
        ...
        body: Padding(
          ...
          child: ListView(children: [
            Card(
              ...
              child: const Padding(
                ...
                child: Column(children: [
                  ...
                  Text('Full access to BizBox SA'),
                  SizedBox(height: 8),
                  Text(
                      'Billed automatically every month. Cancel anytime in Google Play.',
                      textAlign: TextAlign.center,
            ...
            FilledButton.icon(
              ...
              label: Text(_busy
                  ? 'Processing...'
                  : 'Subscribe R99/month via Google Play'),
            ...
            const Text('Alternative: EFT Payment',
            ...
            OutlinedButton.icon(
              ...
              label: const Text('Upload EFT Proof'),
              onPressed: _busy ? null : _uploadEft,
            ),
            const SizedBox(height: 24),
            TextButton.icon(
              ...
              onPressed: () => launchUrl(
                  Uri.parse('https://wa.me/27000000000?text=Subscribe%20BizBox')),
            ),
          ]),
        ),
      ),
...
class _AdminPanelState extends State<AdminPanel> {
  ...
  Future<void> _unlock() async {
    final p = await SharedPreferences.getInstance();
    if (_pin.text.trim() == (p.getString('admin_pin') ?? '')) {
  ...
  Widget build(BuildContext context) {
    ...
    if (!_unlocked) {
      return Scaffold(
        appBar: AppBar(title: Text(_hasPin ? 'Admin Login' : 'Set Admin PIN')),
        body: Padding(
          padding: const EdgeInsets.all(24),
          child: Column(children: [
            Text(
              _hasPin
                  ...
                  : 'First time? Create your private admin PIN.\nOnly YOU will know this.',
              textAlign: TextAlign.center,
            ...
            const SizedBox(height: 16),
            FilledButton(
              onPressed: _hasPin ? _unlock : _setAdminPin,
              child: Text(_hasPin ? 'Unlock' : 'Create PIN & Unlock'),
            ),
          ]),
        ),
      );
    ...
    return Scaffold(
      ...
      body: ListView(padding: const EdgeInsets.all(16), children: [
        Row(children: [
          Expanded(
              child: _statCard(
                  'Businesses', '$totalBiz', Icons.store, Colors.blue)),
          const SizedBox(width: 10),
          Expanded(
              child: _statCard(
                  'Revenue', 'R$revenue', Icons.attach_money, Colors.green)),
        ]),
        const SizedBox(height: 20),
        const Text('All Businesses',
            style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
        const SizedBox(height: 10),
        ..._mock.map((m) => Card(
              child: ListTile(
                leading: Icon(
                    m['paid'] == true ? Icons.check_circle : Icons.warning,
                    color: m['paid'] == true ? Colors.green : Colors.orange),
                title: Text(m['name'] as String),
                subtitle: Text('${m['type']} • ${m['days']} days left'),
                trailing: Wrap(children: [
                  IconButton(
                    ...
                    onPressed: () => ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                            content:
                                Text('Notification sent to ${m['name']}'))),
                  ),
                  IconButton(
